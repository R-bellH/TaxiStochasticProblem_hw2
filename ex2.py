...
def prob_stay(passenger):
    if passenger['destination'] in passenger['possible_goals']:
        return (1 - passenger['prob_change_goal']) + (passenger['prob_change_goal'] / len(passenger['possible_goals']))
    return 1 - passenger['prob_change_goal']
class OptimalTaxiAgent:
    def __init__(self, initial):
        self.initial = initial
        initial_problem=TaxiStochasticProblem(initial)
        # Initialize Markov Decision Process model
        actions =  {}
        for taxi in initial['taxis']:
            actions[taxi] = []
            actions[taxi].append(('drop off', taxi))
            actions[taxi].append(('pick up', taxi))
        ...
        states = [defaultdict(lambda: 0) for _ in range(101)]
        states[0] = [initial_problem]
        for step in states:
            for state in step:
                for act in actions:
                    new_state = deepcopy(state)
                    new_state.result(act)
                    for passenger in state['passengers']:
                        for goal in state['passengers'][passenger]['possible_goals']:
                            new_state=deepcopy()
                            

                    # if new_state not in step:
                    #     step[new_state] = 1-np.prod([prob_stay(passenger) for passenger in new_state.state['passengers']])
                    # for passenger in state['passengers']:
                    #     for goal in state['passengers'][passenger]['possible_goals']:
                    #         new_state = deepcopy(state)
                    #         passenger['destination'] = goal
                    #         if new_state not in step:
                    #             step.append(new_state)


        rewards = lambda x: x.score
        ...
        V = defaultdict(lambda: 0)  # Initialize value function
        pi = defaultdict(lambda: 0)  # Initialize policy function

        # Start value iteration
        for i in range(max_iter):
            max_diff = 0  # Initialize max difference
            V_new = defaultdict(lambda: 0)  # Initialize new value function
            for s in states:
                max_val = 0
                for a in actions:
